...
public class CarService {
    ...
    public List<Record> downReport(String userName, String visitName, String plate, String startTime, String endTime, String visitDept) {
        ...
        sql.append("select userName,visitName,plate,c.num,c.idNO,u.realName,concat(replyDate,' ',replyTime) replayDate,(case cStatus when 'applyConfirm' then '申请中' when 'applySuccess' then '接受访问' when 'applyFail' then '拒绝访问' end)cStatus,visitDept from v_car c left join v_dept_user u on u.id = c.replyUserId where c.id is not null ");
