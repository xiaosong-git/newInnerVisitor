#sql("findApplySucByOrg")
	select cu.id,0 companyId,cu.sectionId,cu.userId,cu.userName,cu.createDate,cu.createTime,
         ...
         from   v_dept_user   du
                 left join   v_dept  d on du.deptId=d.id
                 join   v_org  og on d.org_id=og.id
                 where og.org_code = ?
                 and cu.status = 'applySuc'
                 and (u.authDate = ? or
                 cu.createDate=? and u.isAuth = 'T'
                 UNION all  
                ...
                where org.org_code=#p(org_code)  and u.isAuth = 'T' and DATE_FORMAT(ovu.createDate, '%Y-%m-%d') = ?
#end
